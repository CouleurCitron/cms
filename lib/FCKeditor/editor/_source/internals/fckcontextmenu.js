...
FCKContextMenu._GetGroup = function( groupName )
{
	...
	switch ( groupName )
	{
		...
		case 'Glossary' :
			return new FCKContextMenuGroup( true, this, 'Glossary', FCKLang.GlossaryProperties, true ) ;
		case 'Video' :
			return new FCKContextMenuGroup( true, this, 'Video', FCKLang.VideoProperties, true ) ;	
...
//	if ( this.Groups['Link'] )			this.Groups['Link'].SetVisible( !bIsAnchor && FCK.GetNamedCommandState( 'Unlink' ) != FCK_TRISTATE_DISABLED ) ;
	...
	if ( this.Groups['Glossary'] )		this.Groups['Glossary'].SetVisible( sTagName == 'IMG' && oTag.getAttribute('_fckglossary') ) ;
	if ( this.Groups['Video'] )			this.Groups['Video'].SetVisible( sTagName == 'IMG' && oTag.getAttribute('_fckvideo') ) ;
