...
function UpdateEmbed( e )
{
	...
	SetAttribute( e, 'pluginspage'	, 'https://get.adobe.com/flashplayer/' ) ;
...
// <embed id="ePreview" src="fck_flash/claims.swf" width="100%" height="100%" style="visibility:hidden" type="application/x-shockwave-flash" pluginspage="https://get.adobe.com/flashplayer/">
...
function OpenServerBrowser( type, url, width, height )
{
	...
	if ( oEditor.FCKBrowserInfo.IsIE )
	{
		...
		if (oWindow!=null){
			...
			alert("Une fenêtre à été bloquée par Internet Explorer: autorisez les pop-ups pour ce site.");	
		}
    }
    else
...
function SetUrl( url)
{
	...
	bReturn = XHRConnector.sendAndLoad('/backoffice/cms/utils/swfmetadata.php?file='+url, 'GET', 'foo', this, metaAjax);	
}

function metaAjax(data){
	customMsg = data['request'].responseText;
	
	if ((customMsg!=undefined)	&& (customMsg.indexOf("x")>0)){
		aDim = 	customMsg.split('x');
		width=aDim[0];
		height=aDim[1];
