		...
		if(h.substr(h.indexOf('-')+1,h.length)=='header'){
			...
			h.onclick = function(){
				...
				for(i=0;i<this.c.length;i++){
					...
					anchorName = n+"-header";
					anchorTag = document.getElementById("anchor-"+anchorName);
						
					if((n+'-header')==this.id){		
						//alert(n+" is clicked et sa class is "+ document.getElementById(n+'-header').className);
						
						divOffsetTop = document.getElementById(n+'-header').offsetTop;

						if (document.getElementById(n+'-header').className.indexOf(document.getElementById(n+'-header').tc) > -1){
							//alert("déjà ouvert");
							cl(document.getElementById(n+'-content'));
							cc(document.getElementById(n+'-header'),'');
							//if (anchorTag != undefined){
							//	anchorTag.style.setProperty('display','none',null);
							//};
						}
						else{
							...
							//if (anchorTag != undefined){
							//	anchorTag.style.setProperty('display','block',null);
							//};

							setTimeout(window.scroll, 250, 0, Math.floor(divOffsetTop)+20);
							
							//document.location.href="#"+anchorName;
							
						}						
					}else{
						//alert(n+" is NOT clicked, so is closed");
						cl(document.getElementById(n+'-content'));
						cc(document.getElementById(n+'-header'),'');
						//if (anchorTag != undefined){
						//	anchorTag.style.setProperty('display','none',null);
