	...
	if(window.opener || window.tinyMCEPopup || $.urlParam('field_name') || $.urlParam('CKEditorCleanUpFuncNum') || $.urlParam('CKEditor')) {
	 ...
	 // tinymce 4 and colorbox
	 	if($.urlParam('field_name')){
			...
			if (parent.document.getElementById($.urlParam('field_name'))!=null){
	 			parent.document.getElementById($.urlParam('field_name')).value = url;
			}
			else{
				opener.document.getElementById($.urlParam('field_name')).value = url;
			}
			...
			if (window.opener) {
				// Popup
				var field = window.top.name; 
				window.opener.CKEDITOR.tools.callFunction($.urlParam('CKEditorFuncNum'), url, 'w', 'h', field);
			} else {
				// Modal (in iframe)
			...
			if(data['Properties']['Width'] != ''){
				...
				var field = window.top.name; 			
				window.opener.SetUrl(p,w,h, field);
			} else {
				var field = window.top.name; 	
				window.opener.SetUrl(url, 'w', 'h', field);
			}		
		}
