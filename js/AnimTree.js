...
this.ito.reverse();this.pat=setInterval("Menus."+this.id+".o()",TreeParams.TIME_DELAY);if(Browser.id.win9x&&!Browser.id.NS6)this.pat2=setInterval("Menus."+this.id+".o()",TreeParams.TIME_DELAY);this.oB.dp=true;},cS:function(){if(this.ic) return;if(this.io){this.oE();}else{this.io=false;this.cur=0;this.itc=new Array();for(var i=this.al.length-1,n=0;i>0;i--)
if(this.al[i].style.display=="block")
this.itc[n++]=this.al[i];}this.itc[this.itc.length]=this.el;this.pat=setInterval("Menus."+this.id+".cl()",TreeParams.TIME_DELAY);if(Browser.id.win9x&&!Browser.id.NS6)this.pat2=setInterval("Menus."+this.id+".cl()",TreeParams.TIME_DELAY);this.ic=true;this.oB.dp=false;},o:function(){this.ito[this.cur].style.display="block";if(++this.cur==this.ito.length)
this.oE();},cl:function(){this.itc[this.cur].style.display="";if(++this.cur>=this.itc.length)
this.cE();},oE:function(){clearInterval(this.pat);clearInterval(this.pat2);this.io=false;this.itc=this.ito.reverse();this.cur=this.ito.length-this.cur;if(!TreeParams.OPEN_MULTIPLE_MENUS&&this.c.aM!=this)
this.cS();this.c.aM=this;},cE:function(){clearInterval(this.pat);clearInterval(this.pat2);this.ic=false;if(this.cur>=this.itc.length)
